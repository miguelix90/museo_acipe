...
<div class="bg-white rounded-lg shadow p-6">
    <h2 class="text-xl font-semibold mb-6">Nueva Sala</h2>
    ...
    <form action="{{ route('admin.salas.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="space-y-6">
            ...
            <div>
                ...
                <select name="exposicion_id" id="exposicion_id" required
                    ...
                    @foreach($exposiciones as $exposicion)
                        <option value="{{ $exposicion->id }}" {{ old('exposicion_id') == $exposicion->id ? 'selected' : '' }}>
                            {{ $exposicion->titulo }}
                        </option>
                    @endforeach
                </select>
            ...
            <!-- Audio -->
            <div>
                <label for="audio" class="block text-sm font-medium text-gray-700">Audio/Podcast</label>
                    ...
                    @foreach($libros as $libro)
                        <div class="flex items-center mb-2">
                            <input type="checkbox" name="libros[]" value="{{ $libro->id }}" id="libro_{{ $libro->id }}"
                                {{ in_array($libro->id, old('libros', [])) ? 'checked' : '' }}
